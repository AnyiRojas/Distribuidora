...
const OrderHistory = () => {
  ...
  const fetchOrderHistory = async (documento) => {
    ...
    setError('');
    try {
      const response = await fetch(`https://conection-1.onrender.com/api/historial/${documento}`);
      if (!response.ok) {
  ...
  const cancelarPedido = async (id_pedido) => {
    ...
    if (decoded && decoded.documento) {
      try {
        // Cancelar el pedido
        const cancelResponse = await fetch(`https://conection-1.onrender.com/api/pedidos/cancelar/${id_pedido}`, {
          method: 'PUT',
          headers: {
